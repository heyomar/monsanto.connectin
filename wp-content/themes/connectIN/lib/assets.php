<?php
function offset_load_assets () {
  // TODO: refers to dev stylesheet, rename reference to .min.css for production
  wp_enqueue_style( 'master', get_stylesheet_directory_uri() . '/bundle.css', false );

  wp_register_script( 'footer_scripts', get_stylesheet_directory_uri() . '/bundle.es5.js', array('jquery'), VERSION, true );

  wp_enqueue_script( 'jquery' );
  wp_enqueue_script( 'footer_scripts' );
}
add_action( 'wp_enqueue_scripts', 'offset_load_assets' );
